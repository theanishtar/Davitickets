...
import com.davisys.entity.model.ProfileModel;
...
public class Profile {
	...
	public ResponseEntity<ProfileModel> loadProfile(HttpServletRequest request){
		try {
			...
			ProfileModel profile = new ProfileModel(user.getFull_name(), user.getEmail(),user.getProfile_picture(), user.getPhone(), 
					user.getUser_birtday(), null, user.getGender());
	...
	public ResponseEntity<String>changePass(@RequestBody String newPass,HttpServletRequest request){
		try {
			String email = jwtTokenUtil.getEmailFromHeader(request);
			Users  user = dao.findEmailUser(email);
			user.setUser_password(passwordEncoder.encode(newPass));
			dao.saveAndFlush(user);
