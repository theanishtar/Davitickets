...
public class Login {
	...
	public int pushToShield(HttpServletRequest req, AuthenticationRequest authReq) {
		/*
		 * STATUS 200 login success -> 0 301 account fail -> -2 302 account is banked
		 * (server)-> return -1 303 account is locked (spam) -> return time lock
		 */
		...
		if (shieldFind == null) {
			...
			if (shieldFind.getIp().equalsIgnoreCase(getClientIp(req))) {
				...
				if (shieldFind.getCountErr() >= 5) {
					...
					System.out.println("T Passed: " + minutesPassed);
					if (minutesPassed <= (shieldFind.getCountErr() / 5) + 2 && shieldFind.getCountErr() % 5 == 0) {
						// 3p chưa hết hạn -> không thể Login
						System.out.println(((shieldFind.getCountErr() / 5) + 2) - minutesPassed);
						int checkTime = (int) (((shieldFind.getCountErr() / 5) + 2) - minutesPassed);
						if (checkTime <= 0)
							return doLogin(authReq.getEmail(), req, authReq);
						// trả về lũy tiến theo bội số của những lần đăng nhập sai
						/*
						 * 5l ban 3p 
						 * 10l ban 8p 
						 * 15l ban 15p
						 * 20l ban 24p
						 */
						return (int) (((shieldFind.getCountErr() / 5) + 2) - minutesPassed) * (shieldFind.getCountErr() / 5);
					} else {
	...
	public ResponseEntity<AuthenticationResponse> authLog(@RequestBody AuthenticationRequest authenticationRequest) {
		...
		if (shieldLogin != 0) {
			if (shieldLogin == -1) {
				...
				authRes.setName(
						"Tài khoản của bạn đã bị khóa, hãy liên hệ với Email: Davitickets@gmail.com để được mở khóa!");
		...
		try {
			...
			if (userCheck != null) {
				...
				return ResponseEntity
						.ok(new AuthenticationResponse(userCheck.getFull_name(), authorities, token, null));
			}
		} catch (Exception e) {
			e.printStackTrace();
			return ResponseEntity.status(301).body(null);
